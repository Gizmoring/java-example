...
public class MyStorage {
    ...
    public static void prod_amount_add(Scanner s) {
        ...
        if (isValid(inPutNum)) {
            int cnt = Integer.parseInt(inPutNum);
            counts[foundIdx] += cnt;  // 제품의 수량을 증가
        } else {
            System.out.println("[Eorror] 숫자를 입력해야 합니다.");
            return;
        }
    ...
    public static boolean isValid(String str) {
        String numTable = "0123456789";
        for (char c : str.toCharArray()) {
            if (numTable.indexOf(c) == -1) {
                return false;
            }
        }
        return true;
    ...
    public static void prod_amount_decrease(Scanner s) {
        ...
        //상품 번호, 가격의 인덱스가 같으니까 입력된게 더 크다면

        String inPutNum = s.nextLine();
        if (isValid(inPutNum)) {
            int cnt = Integer.parseInt(inPutNum);
            if (counts[foundIdx] < cnt) {  // 제품의 수량을 증가
                System.out.println("제품의 출고량이 잘못되었습니다.");
            } else {
                counts[foundIdx] -= cnt;
